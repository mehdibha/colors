...
The practical shape of it: red-green vision is essentially _one channel_, and protan and deutan knock it out. Lose it and every hue that depended on it becomes a synonym for its neighbors. You can watch the collapse directly — twelve hues, equally spaced, equal lightness and chroma, nothing separating them but angle:
...
1. A teammate ships a status system where "healthy" is green and "down" is red, same lightness, no icons. Roughly what fraction of male users is likely to struggle, which two deficiency types are responsible, and what's the smallest change that fixes it?
2. "CVD doesn't affect brightness, so if my colors differ in lightness I'm safe." Where exactly is that sentence wrong, and what does it mean for how much you can lean on OKLCH L alone?
3. A user has Windows High Contrast on. Your card uses a background gradient, a drop shadow, and a colored left border to signal "premium." What does the browser do to each of those three, and which one survives to do the signaling?
