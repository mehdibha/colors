...
Now run the orange button through it. `#ff6600` has Y = 0.308. Black brings Y = 0: the ratio is 0.358 / 0.05 = **7.15** — passes AAA. White brings Y = 1: the ratio is 1.05 / 0.358 = **2.94** — fails even the large-text bar. Look at where black's 7.15 came from, though: almost all of it is the denominator bottoming out at the flare constant. Against black, _any_ background at Y ≥ 0.30 scores 7:1 or better automatically. The formula isn't scoring how well black reads on orange — it's scoring how little light black emits.
...
Two hunts worth running. Load **Orange · black**, then **Orange · white**: WCAG ranks black the runaway winner; APCA reverses the ranking, and the black text that sailed through WCAG doesn't clear APCA's body-text bar. Then load **Dark muted** — a pass at 4.52:1, and APCA's Lc −35.3 calls it what your eye sees: below the floor for body text. And note what the orange fight was actually telling you: when the meters disagree, the pair is fragile. The professional answer was never to pick a referee — it was to fix the orange. Darken it two steps to `#c2410c` and white clears _both_ meters (5.18:1, Lc −79.9). A design that needs a favorable referee is the design to change.
...
**WCAG 2 is the normative standard, and the legal one.** US Section 508 incorporates WCAG 2.0 AA; the DOJ's 2024 rule under ADA Title II requires WCAG 2.1 AA for state and local government web content; the EU's EN 301 549 — the standard behind the European Accessibility Act — builds on WCAG 2.1 AA. When a contract, an audit, or a lawsuit says "contrast," it means the ratio and the 4.5:1 from this chapter's second section.
...
1. Walk the orange button through WCAG 2 by hand: Y is 0.308 for the orange, 0 for black, 1 for white. Where does black's 7.15 actually come from — and why does APCA, which soft-clamps the dark end instead of dividing by a propped-up denominator, rank the two buttons the other way?
2. A dark-mode theme's muted text passes 4.5:1 in the audit and users still report it's unreadable. Name the two constructions in WCAG 2's formula that let this pair through, and what APCA responds to that catches it.
3. A teammate proposes switching CI's contrast checks from WCAG 2 to APCA "since WCAG 3 is around the corner." What's right in that sentence, what's wrong, and what should the pipeline do with each score?
