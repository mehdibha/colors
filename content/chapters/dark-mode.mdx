...
1. A teammate proposes dark mode as `L → 1 − L` on every token, "since OKLCH is perceptually uniform." Name three distinct failures — one at the surface steps, one on the accent, one about elevation — and the perception fact behind each.
2. Radix ships the same step 9 hex in light and dark for blue, but different step 11s. Why does the solid survive the room change while the text color can't?
3. In light mode, yellow's text step is a brown; in dark mode it's a vivid yellow with more chroma than the light version. What constraint flipped, and why should this kill any "desaturate everything for dark" rule in your engine?
