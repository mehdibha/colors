...
1. A teammate colors a six-series line chart with steps 4–9 of the accent ramp. Name both clauses of the categorical contract it breaks, and what the false message of the shared hue is.
2. What is the one non-negotiable property of a sequential palette, how exactly does jet violate it (shape of the curve, not just "it's rainbow"), and what does viridis do with hue and chroma while keeping it?
3. Your diverging heatmap ships with a `#f7f7f7` midpoint on a dark dashboard. What does the zero region do visually, what happens to the extremes, and which value from the dark theme should the midpoint be pinned to instead?
