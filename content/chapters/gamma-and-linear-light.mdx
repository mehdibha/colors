...
1. `#808080` and `#bcbcbc` both claim to be "50% gray." In what sense is each one right?
2. A teammate averages two colors channel-by-channel on their hex values to get a midpoint. What did they actually average — and why does the result usually look only slightly wrong instead of catastrophically wrong?
3. Gradients and blurs are coming out with dark, muddy middles. Which ladder is the math running on, and which should it run on?
