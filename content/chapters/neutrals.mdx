...
1. A teammate builds the neutral ramp at C 0 exactly, arguing "gray is gray." What do shipped systems do instead — roughly what chroma range, tinted toward what — and what breaks visually if you push that same dial to C 0.05?
2. Tinting a neutral ramp changes each step's hue and chroma but leaves its lightness alone. Why do the text-contrast contracts barely move, and what does that tell you about which axis the contracts actually live on?
3. Your product ships a jade accent. Which Radix gray is the natural pairing and what measurement backs it — and what does choosing pure `gray` instead trade away, and when is that the right call?
