...
1. `text-gray-500` on white passes 4.5:1. Name two conventional pairs a reader would assume are just as safe that actually fail, and explain what Radix ships that makes the same question answerable without opening a contrast checker.
2. The v4.0 announcement claims more vivid colors via the wider gamut, with the v3 balance kept. What did measurement find for both claims — and what exactly does an sRGB screen do with the 95 values involved?
3. Describe the shape of Tailwind's lightness skeleton — where it's tight, where it fans, and why — and what happened to yellow-500 when the flagship's contrast-anchored ghost re-solved it.
