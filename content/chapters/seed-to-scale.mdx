...
1. A pastel brand pink lands at step 2 by lightness, but the brand meant it as their button color. Name the two slotting rules in play, what each would do with the pastel, and which of the two can put the ladder out of order.
2. The same seed goes into Radix's custom palette generator and into Material's default dynamic scheme. For each: which of the seed's three OKLCH numbers survives, and where — if anywhere — does the exact pasted color appear in the output?
3. Snap and verbatim each keep one promise by breaking another. Name what each keeps, what each costs, and who controls that cost — the engine, or whoever picked the brand color.
