...
1. Geist's gray ladder isn't monotonic — name one wobble and explain why it's evidence of health rather than a bug. What are the step numbers actually encoding?
2. The error button ships on red-800 instead of red's designated high-contrast step. What measurement forces the detour, and what is the only contrast promise Geist puts in writing for the whole system?
3. Geist and Radix both hand-tune every value. Name the scope decision that makes this survivable for Vercel, the three Geist moves worth stealing anyway, and the property of dotUI's product that rules out the strategy wholesale.
