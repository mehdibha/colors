...
1. A constant-hue OKLCH yellow ramp has perfect steering by chapter 5's standards, yet its dark steps read olive. What exactly went wrong — and why does bending the darks toward orange fix it while bending toward green makes it worse?
2. Turning a blue solid and a yellow solid by the same 20° produces the same ΔEOK. What does that number measure, what doesn't it measure, and why does that force hue policy to be set per hue family instead of one global number?
3. Name the two independent reasons dark yellows bend toward orange — one about naming, one about the gamut tent — and explain which direction a dark blue drifts and what it buys there.
