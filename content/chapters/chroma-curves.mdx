...
1. A brand guide mandates "all scale steps at C 0.18 for consistency." Give both reasons this ships something other than what it says — the one physics enforces at the scale's ends, and the one taste enforces even where the gamut would allow it.
2. An engine stores its chroma curve as percentages of the gamut ceiling. Blue looks great; green comes out fluorescent at the same percentages. What fact about the two hues' ceilings explains the difference, and why doesn't the same problem appear at the near-white steps?
3. Your absolute chroma curve fits blue, but at hue 110 half the steps clamp. Name three genuinely different fixes and the strategy each one belongs to.
